...
export default function ManagerDashboard() {
  const { products, customers, sales, settings } = useApp();
    ...
    >
      <motion.div variants={fadeUp} className="page-head">
        <div>
          ...
          <p className="page-sub">Pramukh Hardware ERP • Store operations & inventory alerts</p>
        </div>
                ...
                {sales.slice(0, 8).map(s => {
                  ...
                  return (
                    <tr key={s.id} className="clickable" onClick={() => navigate(`/sales/${s.id}`)}>
                      <td className="cell-main">{s.invoiceNo}</td>
