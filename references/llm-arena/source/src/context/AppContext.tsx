...
export function AppProvider({ children }: { children: ReactNode }) {
  const [state, setState] = useState<AppState>(() => {
    try {
      ...
      if (saved) {
        ...
        return { ...parsed, searchQuery: '', sidebarOpen: false };
      }
    } catch {}
    return {
      currentUser: null,
  ...
  useEffect(() => {
    if (state.currentUser) {
      ...
      localStorage.setItem('bms_state', JSON.stringify(toSave));
    }
  }, [state]);
  ...
  const logout = useCallback(() => {
    localStorage.removeItem('bms_state');
  ...
  return (
    <AppContext.Provider value={{
      ...
      updateSettings, setSearchQuery, setSidebarOpen,
    }}>
      {children}
    </AppContext.Provider>
  );
